...
\midrule
Factual schema lookup & Identify correct byte range
    for birthweight & 0.10 & 0.10 & \emph{compute} \\
...
\subsection*{Block D --- Methodological Consensus on Projection
Approach (30 minutes)}

The data challenge requires projections to 2030. The NCHS data
ends in 1986. The analytical chain is NCHS $\rightarrow$ SEER
$\rightarrow$ SEDAC. Teams query the ensemble on three candidate
modeling strategies:

\textbf{Approach~A --- County-level linear trend extrapolation.}
Fit a per-county linear trend to the 1969--1986 NCHS baseline;
extend to 2030 using SEER population denominators. Required
assumption: secular trend is approximately stationary over the
extrapolation period. Key risk: 44-year extrapolation from a
17-year baseline with no structural break correction.

\textbf{Approach~B --- Multilevel model with county random effects.}
Fit a mixed model with county random effects on the NCHS period;
use SEER projections as denominators for forward extrapolation.
Required assumption: random-effect variance structure is stationary
beyond the data period. Key risk: partial pooling stabilizes sparse
counties but does not improve extrapolation validity.

\textbf{Approach~C --- Age-period-cohort model linked to SEER
denominators.} Separate age, period, and cohort effects with SEER
denominators applied at each projection step. Required assumption:
identification constraints are defensible and explicitly justified.
Key risk: APC models are not uniquely identified without additional
constraints; the choice of constraint is a substantive modeling
decision.

For each approach, the ensemble is asked to evaluate: (a)~the key
assumptions required for validity, (b)~the identifiability
conditions, (c)~the major failure modes in this specific
extrapolation context, and (d)~published precedent for this type
of vital statistics projection. Teams apply the $n_{\min}$ value
computed for methodological judgment tasks.
...
take full responsibility for the work. The statement must be
consistent with the completed invalidation log.
...
\begin{landscape}
    ...
    % -------------------------------------------------------
    % Row 3: Verified Data Query: Texas Birth Count (15%)
    ...
    % -------------------------------------------------------
    % Row 4: Invalidation Log (10%)
    % Maps to Assessment Item 4 (Sections 7.2 and 7.3)
    % -------------------------------------------------------
    \textbf{4.\ Invalidation Log}
    \newline\textbf{(10\%)}
    &
    ...
    % -------------------------------------------------------
    % Row 5: Race Variable Handling Strategy (10%)
    % Maps to Assessment Item 5 (Section 7.2 Block C)
    % -------------------------------------------------------
    \textbf{5.\ Race Variable Handling Strategy}
    \newline\textbf{(10\%)}
    &
    ...
    % -------------------------------------------------------
    % Row 6: Methodological Consensus Memo (15%)
    % Maps to Assessment Item 6 (Section 7.2 Block D)
    % -------------------------------------------------------
    \textbf{6.\ Methodological Consensus Memo}
    \newline\textbf{(15\%)}
    &
    ...
    % -------------------------------------------------------
    % Row 7: NIH Grant Review Analysis (10%)
    % Maps to Assessment Item 7 (Section 7.3 Block A)
    % -------------------------------------------------------
    \textbf{7.\ NIH Grant Review Analysis}
    \newline\textbf{(10\%)}
    &
    ...
    % -------------------------------------------------------
    % Row 8: JHS Specific Aims Draft (10%)
    % Maps to Assessment Item 8 (Section 7.3 Block B)
    % -------------------------------------------------------
    \textbf{8.\ JHS Specific Aims Draft}
    \newline\textbf{(10\%)}
    &
    ...
    % -------------------------------------------------------
    % Row 9: Final Disclosure Statement (5%)
    % Maps to Assessment Item 9 (Section 7.3 Block B, closing)
    % -------------------------------------------------------
    \textbf{9.\ Final Disclosure Statement}
    \newline\textbf{(5\%)}
    &
