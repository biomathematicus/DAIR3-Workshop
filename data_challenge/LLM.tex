...
This chapter documents the configuration of a local Large Language Model
(LLM) serving infrastructure using Ollama, with specific attention to
two deployment scenarios: a \emph{single-machine} setup suitable for
individual development, and a \emph{two-machine} setup that separates
the application stack from the LLM serving layer. The examples use a
Dell Precision 7770 workstation as the application host and an NVIDIA
Spark as the dedicated LLM server, both connected through a university
network.

The choice of Ollama as the serving layer is motivated by three
properties relevant to the \textsc{Alice} stack. First, Ollama exposes
an OpenAI-compatible REST~API at \texttt{/v1/chat/completions}, which
integrates directly with the \texttt{ProviderAdapter} protocol already
implemented in \textsc{Max}. Second, Ollama has first-class support for
unified-memory architectures such as the NVIDIA GB10 SoC found in the
Spark, where CPU and GPU share the same memory pool. Third, Ollama
manages model lifecycle (loading, eviction, and context
allocation) through a small set of environment variables that translate
cleanly to \texttt{systemd} service overrides, preserving production
parity.
...
\begin{itemize}
  ...
  \item \textbf{Role:} Runs the \textsc{Alice} FastAPI application,
        PostgreSQL~16, and the \textsc{Hudo}/\textsc{Max} agent
...
You will receive an API key for OpenAI and Anthropic during our first session. You can get your own API for free at \href{https://groq.com}{Groq}. You need to create three API key variables following the procedure described below: \texttt{OPENAI\_API\_KEY}, \texttt{ANTHROPIC\_API\_KEY}, \texttt{GROQ\_API\_KEY}. Test the programs located at \texttt{resources/unit\_7/FOO/} in the DAIR3-Workshop repository: \texttt{agentGroq.py}, \texttt{agentGPT.py}, and \texttt{agentClaude.py}.
