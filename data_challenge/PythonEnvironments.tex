...
\label{sec:dair3-libraries}

With the DAIR3 environment active, install the required libraries for
the data challenge:
...
\begin{lstlisting}[style=windowsStyle, caption={Installing \texttt{uv} on Windows}]
...
\begin{lstlisting}[style=linuxStyle, caption={Installing \texttt{uv} on Linux/WSL}]
...
\begin{lstlisting}[style=windowsStyle, caption={Creating a uv-managed environment (Windows)}]
uv venv C:\uvenv\DAIR3 --python 3.12
\end{lstlisting}

\begin{lstlisting}[style=linuxStyle, caption={Creating a uv-managed environment (Linux/WSL)}]
uv venv ~/uvenv/DAIR3 --python 3.12
\end{lstlisting}
...
\begin{lstlisting}[style=windowsStyle]
C:\uvenv\DAIR3\Scripts\activate.bat
\end{lstlisting}

\begin{lstlisting}[style=linuxStyle]
source ~/uvenv/DAIR3/bin/activate
\end{lstlisting}
...
\begin{lstlisting}[style=inlineStyle, caption={Installing packages with uv}]
...
\begin{lstlisting}[style=inlineStyle, caption={Minimal \texttt{pyproject.toml}}]
[project]
name = "my-project"
version = "0.1.0"
...
\begin{lstlisting}[style=windowsStyle, basicstyle=\footnotesize\ttfamily, caption={\texttt{ActivateUVEnv.bat}: uv environment launcher (Windows)}]
@echo off
setlocal
...
\begin{lstlisting}[style=linuxStyle, basicstyle=\footnotesize\ttfamily, caption={\texttt{ActivateUVEnv()} shell function for \textasciitilde/.bashrc}]
# -------------------------------------------------------
...
\begin{itemize}
  ...
  \item \textbf{Level~1 scripting}: writing reusable launcher scripts
        ...
        one-liners (\texttt{DAIR3.bat}, \texttt{DAIR3.sh}) that
