...
\texttt{hyperref} (clickable links). These are already loaded in the
shared preamble \texttt{DAIR3Config.tex} for all documents in this
repository; students writing their own documents will need to load
them explicitly.
...
package, which is already loaded in \texttt{DAIR3Config.tex}. The
\texttt{pdflatex} compiler accepts PDF, PNG, and JPG image formats.
...
\texttt{tabular} environment. The \texttt{booktabs} package (already
loaded in \texttt{DAIR3Config.tex}) provides clean horizontal rules.
